...
public class ItemController {
    ...
    public Mono<String> getItems(
        ...
        return itemService.getItems(search, sortType, pageNumber, pageSize)
                .flatMap(item ->
                        // Получаем ID пользователя. Если его нет (аноним) - возвращаем пустой Mono
                        ReactiveSecurityContextHolder.getContext()
                                .map(ctx -> ctx.getAuthentication().getName())
                                .flatMap(name -> cartService.getItemCount(item.getId()))
                                .defaultIfEmpty(0) // Если аноним или в корзине 0 - будет 0
                ...
                .map(tuple -> {
                    ...
                    model.addAttribute("paging", PagingDto.builder().pageSize(pageSize).pageNumber(pageNumber)
                            .hasPrevious(pageNumber > 1).hasNext(tuple.getT2() > (long) pageNumber * pageSize).build());
                    return "items";
                });
    }

    @PostMapping("/items")
    public Mono<String> updateCartFromList(@Valid CartRequest request) {
        return cartService.updateItem(request.getId(), CartAction.fromString(request.getAction()))
                .thenReturn(String.format("redirect:/items?search=%s&sort=%s&pageNumber=%d&pageSize=%d",
                        request.getSearch() != null ? request.getSearch() : "", request.getSort(),
